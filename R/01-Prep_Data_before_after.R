...
for(i in 1: nrow(sample_dat)) {
  ...
  # Extract rows of burst 20h before sample (represents behaviour after GC spike)
  # OR  40h-20h before sample (represents behaviour after GC spike)
  row_burst <- data.frame()
  for(j in c('before', 'after')) {
    if(j == 'before') {
      sub_burst <- loc_dat %>%
        filter(animal_ID == sample_dat[i ,]$animal_ID &
                 time_lmt %in% seq(loc_dat[row_ind ,]$time_lmt - lubridate::hours(40), 
    ...
    # Add col to identify locations before/after
    sub_burst <- sub_burst %>%
      mutate(sample_sequence = j)
    # Bind together
    row_burst <- rbind(row_burst, sub_burst)
  }
  row_burst <- row_burst %>%
...
hormone_dat <- all_bursts %>% 
  ...
  select(animal_ID, collar_ID, uid, time_lmt, sample_lmt, yr, month, jday,
         sample_sequence, rel_to_calv, cort_ng_g, t3_ng_g, TOD, lat, long)
